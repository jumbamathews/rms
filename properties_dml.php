...
function properties_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	<script>
		...
		AppGini.current_owner__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['owner'] : $filterer_owner); ?>"};
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(operating_account)%%>', html_attr($row['operating_account']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(operating_account)%%>', urlencode($urow['operating_account']), $templateCode);
		$templateCode=str_replace('<%%VALUE(property_reserve)%%>', html_attr($row['property_reserve']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(property_reserve)%%>', urlencode($urow['property_reserve']), $templateCode);
		$templateCode=str_replace('<%%VALUE(lease_term)%%>', html_attr($row['lease_term']), $templateCode);
	...
	$templateCode .= loadView('properties-ajax-cache', $cache_data);
